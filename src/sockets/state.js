...
function reduceJoin(state, { playerId, nickName, img, now }) {
  ...
  const adminId = state.adminId || playerId;
...
function reduceSubmit(
  ...
  const effects = [{ type: "broadcast", event: "answerAdded", payload: ans }];
...
function reduceNextQuestion(state, { playerId, now, roundMs }) {
  ...
  return startRound({ ...state, qIndex: state.qIndex + 1 }, { now, roundMs });
}
